...
def draw_code_background(
    self, code_flag: bool, word: str, x: int, y: int, position: str
):
    ...
    if code_flag:
        if (
            position == "last" or position == "single"
        ):  # remove last char (whitespace) from background-surface
            word = word[:-1]
        ...
        draw_rounded_rect(
            self.screen,
            pygame.Rect(x - self.code_padding, y, width, height),
            self.color_code_background,
            3,
        )
...
def draw_rounded_rect(
    ...
    if rect.width < 2 * corner_radius or rect.height < 2 * corner_radius:
        raise ValueError(
            f"Both height (rect.height) and width (rect.width) must be > 2 * corner "
